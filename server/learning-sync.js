...
function buildLearningSyncProfile(content, state, user) {
  ...
  return {
    ...
    activity: {
      ...
      selfStudy: selfStudy.lessons.map(lesson => ({ lessonId: lesson.lessonId, lessonVersion: lesson.lessonVersion, studyDay: lesson.studyDay, formalDate: lesson.formalDate, enabledFrom: lesson.enabledFrom, expiresAt: lesson.expiresAt, status: lesson.status, startedAt: lesson.startedAt, completedAt: lesson.completedAt, activeSeconds: lesson.activeSeconds, testSummary: lesson.testSummary })),

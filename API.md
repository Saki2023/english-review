...
英译中判题按语义和数量判断，不要求中文量词与参考译文字面完全一致。对于明确的单数人物身份，`一个` 与 `一位` 都可作为自然量词，例如 `She is a mom.` 的“她是一个妈妈”和 `She is a cook.` 的“她是一位厨师”均按完全正确记录；该放宽不适用于猫、笔、箱子等动物或物品，也不接受 `一双`、`一对`、`两个` 等改变数量的表达。证据修复 v6 会幂等更新既有 AI 历史和当前题组，派生的 AI 得分、弱项与能力分析会据此重新计算。

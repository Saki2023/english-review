...
同步脚本使用教学写入令牌调用 `PUT /api/content/batch`。接口会先验证整个请求，再按 `id` 更新或新增正式词句、按 `day` 更新或新增笔记；重复运行不会产生重复内容，也不会删除请求中未包含的旧正式课程。`previewWords` 使用替换语义，只接受 `currentDay + 1` 的词，自动排除已学英文、重复项和更远日期；正式 `words` 与预习项同 ID 时正式内容优先。响应中的 `previewWords` 是当前保留的预习词数量。课程同步形成新周期时，接口还会清除上一轮 AI 句子池并立即启动每个账号的新 50 条后台生成；响应的 `reviewSentencePool` 会给出是否换轮、账号数、已启动数和等待数。相同 `updatedAt` 与已学内容的重复同步不会换轮或重复生成。
...
每个返回句子都对应一个 `wordId`，并且必须包含该预习词；句子可以组合已经正式学过的单词来帮助记忆，但服务端会拒绝未学过且不在当前预习列表中的英文词。返回句子的 `acceptedChinese` 会按正式词库登记的词义扩展到完整句子，例如正式词条 `pool` 允许“水池/游泳池/泳池”时，`A pool is deep.` 会接受这些词义组成的等义整句。未带程度、比较或否定标记的 `deep` 还会保存并接受自然中文“很深/很深的”，同时保留旧的“是深的/深的”作为兼容答案。该规则同时用于生成、状态保存、刷新恢复、判题和旧预习历史迁移；旧错误解释会被替换并重算预习分数。
...
`GET /api/sync/profile` 使用只读同步令牌鉴权，并按账号名返回学习档案。响应包含课程内容、复习状态、AI 做题历史、按题目保存的 `tutorHistory` 问答记录、已完成的 `previewPracticeHistory` 预习练习、试卷成绩与逐题证据、听写和专项历史、七维能力、错题、薄弱点统计及网站保存的本地教学档案，不包含密码、Cookie、会话令牌或 AI Key。问答记录是疑惑线索，不会自动计入错题。

`previewPracticeHistory` 只导出已经完成的轮次，不包含正在输入的预习草稿。每轮记录预习日、模式、总题数、得分和完成时间；每题记录中英文题目、实际题干、用户答案、参考答案、完整判定、问题词及逐词结果。轮次和题目都固定包含 `formalEvidence: false`，预习答错不会进入正式错题、薄弱点、待复习数量或能力分析。汇总字段为 `summary.previewPracticeRounds`、`previewPracticeQuestions`、`previewPracticeFullyCorrect`、`previewPracticePartiallyCorrect`、`previewPracticeIncorrect` 和 `previewPracticeAverageScore`。

`summary.studyGoalSeconds` 为每日 60 分钟目标，`summary.studyGoalDaysMet` 为已达标天数；`activity.dailyStudyTime` 保存各日期的有效秒数，`activity.studyPlan` 给出六个阶段、目标分钟数以及该阶段能否切换到英语学习窗口继续。
...
英译中判题按语义和数量判断，不要求中文量词与参考译文字面完全一致。对于明确的单数人物身份，`一个` 与 `一位` 都可作为自然量词，例如 `She is a mom.` 的“她是一个妈妈”和 `She is a cook.` 的“她是一位厨师”均按完全正确记录；该放宽不适用于猫、笔、箱子等动物或物品，也不接受 `一双`、`一对`、`两个` 等改变数量的表达。普通 `deep` 句子接受自然中文“很深/很深的”，例如 `a deep pool` 可译为“很深的水池/游泳池”，`A pool is deep.` 可译为“水池/游泳池很深”；规则只在英文没有 `very`、`more`、`too`、`not` 等程度、比较或否定标记时生效，不会全局删除中文“很”。证据修复 v7 会幂等更新既有普通复习、AI 历史、当前/待开始题组和试卷证据，相关得分、弱项与能力分析会重新计算。
...
预生成按每批最多 20 条执行，目标为每个学习同步周期 50 条。正常情况下不需要网页手动调用：`PUT /api/content/batch` 同步当天课程成功后会立即启动后台生成。每批成功后立刻写入该账号的服务器状态；返回的 `pool` 只包含摘要（`date`、`syncKey`、`generatedCount`、`remainingCount`、`targetCount`、`status`），不会把完整句子池塞进浏览器状态请求。刷新、重新登录、换设备、服务重启或自然日期变化后，复习请求都会从同一池直接抽取已保存句子；有当前句型时优先使用同族变式，没有同族变式时也使用其他已学词句生成的池内句子。只有池完全为空时才需要等待。下一次课程同步形成新周期时才清除旧池并启动新的 50 条；同一批课程重复同步不会清除或重复生成。句子池不写入正式词句库。

...
function serveStatic(req, res, url) {
  ...
  fs.stat(filePath, (error, stats) => { if (error || !stats.isFile()) return sendError(res, 404, "not found"); setCommonHeaders(res, mimeType(filePath)); res.setHeader("Cache-Control", filePath.endsWith("index.html") || filePath.endsWith("data.js") || filePath.endsWith("sw.js") ? "no-cache" : "public, max-age=3600"); res.writeHead(200); fs.createReadStream(filePath).pipe(res); });
}

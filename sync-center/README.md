...
- 本次准备同步的学习进度、错题本、每日课程词句、完整自学课程包、最近每日笔记和预习文件；
- 实际上传到网站的文件；
- 从网站下载的 `网站学习档案.json`；
- 独立下载的 `网站预习练习.json`，包含已完成预习练习的逐题记录；
- 网站当前课程天数、词句与笔记总数，以及完整自学课程的完成天数、当前位置、首次作答、订正、未作答和待判统计；
- AI 做题、问答、试卷、待复习、听写、专项训练和能力证据统计；
- 最近 50 次同步记录及失败原因；
- 上传和下载各自的成功状态，以及网络、TLS、超时、HTTP 401/403、HTTP 5xx 或本地资料错误类别。

上传教学档案、上传课程内容、上传 `网站自学课程.json` 和下载网站档案是相互独立的阶段。同步详情会分别显示每个阶段以及实际上传的文件。某个上传阶段失败时，工具仍会尝试下载；只要下载成功，`网站学习档案.json` 就会立即刷新，并把本次结果显示为“部分成功”。临时网络、TLS、超时或 HTTP 5xx 错误会有限重试 3 次，权限错误不会反复请求。

...
- 今日句子复习优先由 AI 按已学词句在后台生成自然变式。每个账号每天维护一个目标 100 条的句子池，按最多 20 条一批写入服务器；刷新、重新登录、换设备或服务重启后会继续复用，当日期或学习内容签名变化时才舍弃并重建。每个后台任务最多运行 10 分钟，避免上游连接卡住后永久 pending；合格句子会保留，只对失败项最多修正 3 轮。网络或上游暂时不可用时每 5 分钟无限重试；内容连续 3 轮仍不合格时停止自动重试并提示手动重试或更换模型，不把固定本地句子冒充为 AI 变式，也不把变式写入正式词句库。

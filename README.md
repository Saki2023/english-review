...
- 今日句子复习优先由 AI 按已学词句生成自然变式。每次当天课程完成并同步成功后，服务器会立即为每个账号在后台建立目标 50 条的新句子池，按最多 20 条一批生成并持久化；刷新、重新登录、换设备、服务重启或自然日期变化都继续复用这批内容，直到下一次课程同步成功才清除旧池并自动生成新的 50 条。重复同步同一批课程不会重复清空或调用 AI。每个后台任务最多运行 10 分钟；合格句子立即保留，失败项最多修正 3 轮。网络或上游暂时不可用时每 5 分钟重试；内容连续 3 轮仍不合格时停止自动重试并提示手动重试或更换模型，不把固定本地句子冒充为 AI 变式，也不把变式写入正式词句库。

...
- “今日复习”和“AI 出题”均按整组作答：一次只显示一题，最后一题后先进入只含题目和用户答案的核对页；确认后才统一批改并一次性展示整组结果，草稿、核对和失败重试不会提前写入成绩或错题。
- 可由学习窗口提前预装多天“出门完整自学课程”，用户手动开启后按日、按六阶段、一次一个步骤学习；暂停、草稿、首次答案、订正和提问跨设备持久化。未来课程词句始终保持待学习，只有当天六阶段全部完成时才原子转为正式已学并安排首次复习。
- 可在联网时为当前账号主动准备出门离线包，保存最多 14 天自学课程、当前每日预习/预习练习和最多 20 个已生成 AI 题组，容量上限 6 MiB。只有准备过离线包且未退出的同一账号可在断网后进入；离线作答只保存草稿和待同步操作，不在本机伪造正式得分、错题或能力证据，恢复联网后按稳定 ID 严格顺序重放并以服务器结果为准。实时 AI 生成仍需要联网。
- 首页提供按顺序推进的每日 60 分钟学习计划：复习、拼读词汇、句子结构、阅读翻译、订正和预习各有明确任务与入口，完成一个阶段后才解锁下一个阶段；有效时长按账号保存并同步给本地教学 AI。
- AI 根据已学词句、历史作答、本地教学重点和薄弱点生成题目。
- AI 新题组按账号追加到持久化 FIFO 队列，不覆盖正在作答的题组；后续组只显示模型、强度、题数、时间和状态，当前结果页点击“开始下一组”后才打开队首题目。
...
- 独立“发音课”用中文口型、舌位和气流说明讲解元音、辅音、清浊音、音节与重音；提供 20 个元音和 24 个辅音的分类参考表及慢速示范词。
- “每日预习”保留近期预习文档；独立“预习单词”只显示当前课程紧邻下一天且尚未学过的词，并提供音标、中文说明和慢速喇叭。
- 今日复习、听写、能力统计和本地学习档案只使用正式已学内容；预习词在正式课程同步后才转为已学词。
- 今日句子复习优先由 AI 按已学词句生成自然变式。每次当天课程完成并同步成功后，服务器会立即为每个账号在后台建立目标 50 条的新句子池，按最多 20 条一批生成并持久化；刷新、重新登录、换设备、服务重启或自然日期变化都继续复用这批内容，直到下一次课程同步成功才清除旧池并自动生成新的 50 条。句子池可展开查看真实英文，按需显示中文，支持中英文搜索、10/20/50 分页；每句显示正式做题次数、答对、答错、正确率和最近练习时间，并可按上海时区日期范围筛选及稳定排序。查看和筛选只读，不改变顺序、分配、掌握度或证据。重复同步同一批课程不会重复清空或调用 AI。每个后台任务最多运行 10 分钟；合格句子立即保留，失败项最多修正 3 轮。网络或上游暂时不可用时每 5 分钟重试；内容连续 3 轮仍不合格时停止自动重试并提示手动重试或更换模型，不把固定本地句子冒充为 AI 变式，也不把变式写入正式词句库。
- 网站证据同步回本地教学窗口，本地学习计划也可同步给网站 AI。
- 提供本地“英语学习同步中心”，可查看每次同步的上传/下载内容、网站统计和最近同步历史，并可安装每日自动同步任务。
...
更推荐使用可视化同步中心。在完整工作区中双击 `学习同步\启动同步中心.cmd`，或直接运行：

```powershell
powershell -ExecutionPolicy Bypass -File ".\每日英语复习\sync-center\同步中心.ps1"
```

同步中心会显示本次准备同步、实际上传、从网站下载的文件，以及网站当前课程天数、词句和笔记总数、AI 做题、问答、预习练习、试卷、待复习、听写、专项训练和能力证据统计；最近 50 次结果保存在工作区外的 `学习同步\同步记录`。点击“安装每日自动任务”后，Windows 会每天 03:00 以无界面模式执行同步。界面和记录都不会显示账号密码、Cookie、API Key 或同步令牌。

“立即同步”保持完整的双向正式同步；“仅同步预习练习”只下载网站中已完成的预习练习到 `学习同步\网站预习练习.json`，不读取或上传本地课程、笔记、学习进度和错题。命令行可在同步脚本后增加 `-PreviewPracticeOnly`。完整正式同步仍会在 `网站学习档案.json` 中包含预习练习记录。
...
2. 上传本地 `错题本.md`。
3. 上传最近三份每日笔记。
4. 上传最新及近期预习，并从最新预习的“单词 / 发音 / 中文”表格解析紧邻下一课的预习词。
5. 读取 `学习同步\网站课程内容.json`，幂等更新正式词句、结构化学习笔记和预习词；同步中心会明确显示预习词数量。
6. 下载网站复习、AI 做题与问答、已完成预习练习的逐题详情、试卷、听写、专项、能力和薄弱点档案；预习练习只作为非正式回顾信息。
7. 写入 `学习同步\网站学习档案.json`。
...
“发音课”的喇叭播放包含目标音的英文示范词，不是孤立音标录音。不同设备的系统英语声音可能略有差别；页面默认按当前课程的美式标注讲解，并同时提示常见英式或词典写法。

...
同步中心会显示本次准备同步、实际上传、从网站下载的文件，以及网站 AI 做题、问答、试卷、待复习、听写、专项训练和能力证据统计；最近 50 次结果保存在工作区外的 `学习同步\同步记录`。点击“安装每日自动任务”后，Windows 会每天 03:00 以无界面模式执行同步。界面和记录都不会显示账号密码、Cookie、API Key 或同步令牌。

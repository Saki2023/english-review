...
test("sync center provides a safe visible report and headless entry point", () => {
  ...
  assert.match(script, /uploadSuccess/);
  assert.match(script, /downloadSuccess/);
  assert.match(script, /partialSuccess/);
  assert.match(script, /compatibilityTransportUsed/);
  assert.match(script, /兼容 HTTPS 通道/);
  assert.match(script, /errorCategory/);
  assert.match(script, /-StatusPath/);
  assert.match(script, /上传阶段：\$uploadState/);
  assert.match(script, /下载阶段：\$downloadState/);

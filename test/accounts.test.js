...
test("SSH-created account can log in and public registration is absent", async () => {
  ...
  try {
    ...
    const appResponse = await fetch(`${baseUrl}/app.js?v=33`);
    ...
    assert.match(appSource, /sw\.js\?v=33/);
    const pronunciationResponse = await fetch(`${baseUrl}/pronunciation-data.js?v=33`);

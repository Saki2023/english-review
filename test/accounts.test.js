...
test("SSH-created account can log in and public registration is absent", async () => {
  ...
  try {
    ...
    assert.match(html, /pronunciation-data\.js\?v=72/);
    ...
    const appResponse = await fetch(`${baseUrl}/app.js?v=72`);
    ...
    assert.match(appSource, /sw\.js\?v=72/);
    const pronunciationResponse = await fetch(`${baseUrl}/pronunciation-data.js?v=72`);

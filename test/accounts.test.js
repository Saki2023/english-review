...
test("SSH-created account can log in and public registration is absent", async () => {
  ...
  try {
    ...
    assert.match(html, /pronunciation-data\.js\?v=75/);
    ...
    const appResponse = await fetch(`${baseUrl}/app.js?v=75`);
    ...
    assert.match(appSource, /sw\.js\?v=75/);
    const pronunciationResponse = await fetch(`${baseUrl}/pronunciation-data.js?v=75`);

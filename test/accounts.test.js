...
test("SSH-created account can log in and public registration is absent", async () => {
  ...
  try {
    ...
    const appResponse = await fetch(`${baseUrl}/app.js?v=32`);
    ...
    assert.match(appSource, /sw\.js\?v=32/);
    const pronunciationResponse = await fetch(`${baseUrl}/pronunciation-data.js?v=32`);

...
test("service worker serves cached static assets without touching the network and bypasses APIs", async () => {
  ...
  const cache = worker.cacheFor("daily-english-review-v77");
  const asset = new Request(`${ORIGIN}/app.js?v=77`);
...
test("install precaches the complete v77 app shell and immediately activates the worker", async () => {
  const fetched = [];
  ...
  assert.deepEqual(await worker.caches.keys(), ["daily-english-review-v77"]);
  assert.equal(fetched.length, 19);
  assert.equal(new Set(fetched).size, 19);
  assert.ok(fetched.includes("/index.html"));
  assert.ok(fetched.includes("/app.js?v=77"));
  assert.ok(fetched.includes("/offline-store.js?v=77"));
  assert.ok(fetched.includes("/vendor/lucide.min.js?v=77"));

...
test("formal review batch start has bounded recovery and an explicit retry gate", () => {
  ...
  const server = read("server.js");
  assert.match(html, /review-batch-client\.js\?v=71/);
  assert.match(serviceWorker, /review-batch-client\.js\?v=71/);
...
test("pronunciation lesson lists and filters reference sounds without pretending word speech is isolated IPA", () => {
  ...
  assert.match(serviceWorker, /pronunciation-data\.js\?v=71/);
});
...
test("remote state saves are narrow, serialized, and skip server-authoritative response echoes", () => {
  ...
  assert.match(html, /state-sync-client\.js\?v=71/);
  assert.match(serviceWorker, /state-sync-client\.js\?v=71/);
...
test("PWA client assets consistently use the displayed cache version 71", () => {
  ...
  assert.equal(displayedVersion[1], "71");
  assert.match(serviceWorker, /const CACHE_NAME = "daily-english-review-v71"/);
});
...
test("offline travel mode is explicit, account-bound, FIFO replayed, and never caches private API responses", () => {
  ...
  assert.match(html, /offline-store\.js\?v=71/);
  assert.match(html, /offline-learning\.js\?v=71/);
  assert.match(html, /offline-ai\.js\?v=71/);
  assert.match(html, /offline-replay\.js\?v=71/);
  ...
  assert.match(serviceWorker, /offline-store\.js\?v=71/);
  assert.match(serviceWorker, /offline-replay\.js\?v=71/);
...
test("daily study plan offers six free-choice projects and records sixty minutes across web and learning-window work", () => {
  ...
  assert.match(html, /study-time\.js\?v=71/);
  assert.match(serviceWorker, /study-time\.js\?v=71/);
});
